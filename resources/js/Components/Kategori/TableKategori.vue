<script setup>
    ...
    function saveChanges() {
        form.put(`/dashboard/kategori/${form.id}`, {
            ...
            onError: () => {
                alert('Failed to update kategori data.');
            },
        });
    }
...
<template>
    ...
    <div v-if="isEditModalOpen"
                ...
                <div class="bg-white rounded-lg p-5 w-full max-w-md">
                    <h4 class="text-lg font-semibold mb-4">Edit Kategori</h4>
                    <form @submit.prevent="saveChanges">
                        <div class="mb-4">
                            <label for="kategori" class="block text-sm font-medium text-gray-700">Kategori
                            </label>
                            <input type="text" v-model="form.kategori" class="mt-1 p-2 w-full border rounded" required />
                            <div class="text-danger text-xs" v-if="errors.kategori">{{ errors.kategori }}</div>
                        </div>
